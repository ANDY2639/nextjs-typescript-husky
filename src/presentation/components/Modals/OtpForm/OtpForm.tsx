      ...
      <ModalContent>
        <ModalBody className="flex flex-col gap-8 p-8">
          <header className="flex items-center justify-between">
            <h3 className="text-large not-italic font-bold flex-grow">Código seguro</h3>
            <Button
              className="bg-content1 hover:bg-content1 min-w-0 w-auto h-auto"
              isIconOnly
              aria-label="Close"
              radius="full"
              onPress={handleClose}
          ...
          <section className="flex flex-col gap-6">
            <p className="text-small not-italic font-regular">
            ...
            >
              <div className="flex flex-col gap-2 items-center w-full">
                ...
                {!isExpiredOtp && otpExpiredDate && (
                  <p className="text-center text-tiny not-italic font-regular">
                    Recuerda que el código tiene una duración de<br />
                    <span className="text-tiny not-italic font-medium text-primary">
                      <Countdown
              ...
              {isExpiredOtp ? (
                ...
                >Solicitar código</BaseButton>
              ) : (
                <FormButton
                  disabled={(values: OtpFormValues) => values.otpCode.length !== 6}
                >Enviar</FormButton>
              )}
            </FormProvider>
          </section>

          <footer className="text-center text-tiny not-italic font-regular">
            En caso de no recibir el código de seguridad, ingresa{" "}
            <Link className="text-tiny not-italic font-regular text-primary underline" href="/">aquí</Link>
          </footer>
        </ModalBody>
      </ModalContent>
    </Modal>
